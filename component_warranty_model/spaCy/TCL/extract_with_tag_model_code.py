...
tcl_regex_patterns = {
    "HD & FHD": r"^(L)?[\s-]*\d{2}[\s-]*([Ss]|[Dd]|[Gg])[a-zA-Z0-9\-()+]*$",
    "4K": r"^\d{2}[\s-]*([Pp]|[Cc]|[Qq])[6-9][a-zA-Z0-9\-()+]*$",
}


# File paths
input_file_path = "component_warranty_model/Data/TCL/extracted_models_tcl.xlsx"
input_data_sheet_name = "Sample Data 004"
output_data_sheet_name = "Extracted Models 005"

# Load the spaCy model
nlp = spacy.load("component_warranty_model/spaCy/TCL/fine_tune_model")
...
def match_tags(model_code):
    tags = [
        tag
        for tag, pattern in tcl_regex_patterns.items()
