...
file_path = 'component_warranty_model/Data/Sony/extracted_models_sony.xlsx'
output_file_path = 'component_warranty_model/Data/Sony/output_with_matches.xlsx'

df = pd.read_excel(file_path, sheet_name='Extracted Models 004')

def match_tags(model_code):
    tags = [tag for tag, pattern in sony_regex_patterns.items() if re.match(pattern, str(model_code.strip().replace(' ','')))]
    return tags
